...
from utils import (
    ...
    create_transforms, maxSeg_repeat3, maxSeg_around3, maxSeg_around3_sample, # collate_like_drfuse,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_size', type=int, default=384) # resnet같은거는 224

    
    args = parser.parse_args()

    args.exp_name = input("Specify the note for this run: ") # experiments나 wandb 에 사용할 이름. 귀찮으면 날짜랑 시간같은걸로 자동으로 생성되게 바꾸세요.
    ...
    train_dataset.start()
    valid_dataset.start()
    train_dl = DataLoader(train_dataset, args.batch_size, shuffle=False, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size), # maxSeg_around3, 
                          pin_memory=True, num_workers=args.num_workers, drop_last=True, sampler=sampler)
    val_dl = DataLoader(valid_dataset, args.batch_size, shuffle=False, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size), # maxSeg_around3, 
                        pin_memory=True, num_workers=args.num_workers, drop_last=False)

    model = DrFuseTrainer(args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset) # label_names=['HematomaExpansion', 'nonHE']
    ...
    #                                     verbose=False,
    #                                     mode="max")
    # 원래 아래 early stopping 방식이 국룰인데, multi modal 학습이 워낙 불안정하니까, 일단 위 방식을 주석 해제해놓음
    ...
    test_dl_paired = DataLoader(dataset, batch_size=5, shuffle=False, num_workers=12, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size, test=True))# maxSeg_around3_test
    ...
    # model = DrFuseTrainer.load_from_checkpoint(os.path.join(save_path, 'checkpoint.ckpt'), args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset)
    model = DrFuseTrainer.load_from_checkpoint(checkpoint_callback.best_model_path, args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset)
    ...
    # print(shapClinics.shape)
    for param in model.model.parameters():
        if not param.requires_grad:
            print('param')
            param.requires_grad = True
    # x = x.cpu()# .numpy()
    model.model.ehr_model.shap = True
    explainer = shap.DeepExplainer(model.model.ehr_model, shapClinics_train) # (shapModelWrapper(self.model.ehr_model.cpu()), x)
    ...
    # print(feature.columns, shapClinics_train.shape, shapClinics.shape, shap_values.shape)
