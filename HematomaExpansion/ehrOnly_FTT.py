...
from models import FTT_trainer
...
from utils import (
    ...
    create_transforms, maxSeg_repeat3, maxSeg_around3, FTT,# maxSeg_around3_test, collate_like_drfuse,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ehr_n_layers', type=int, default=4)
    parser.add_argument('--ehr_n_head', type=int, default=8)
    parser.add_argument('--hidden_size', type=int, default=192)
    parser.add_argument('--dropout', type=float, default=0.3)
    parser.add_argument('--lr', type=float, default=1e-04)
    parser.add_argument('--wd', type=float, default=0, help='weight decay') # 1e-05
    ...
    parser.add_argument('--cat_ind', type=int, default=[0, 2, 5, 6, 7, 13], nargs='+', help='separate with space bar')
    parser.add_argument('--cat_cardinalities', type=int, default=[2, 2, 5, 6, 7, 2], nargs='+', help='separate with space bar')
    parser.add_argument('--cont_ind', type=int, default=[1, 3, 4, 8, 9, 10, 11, 12], nargs='+', help='separate with space bar')
    
    args = parser.parse_args()
    assert len(args.cat_ind) + len(args.cont_ind) > 0
    ...
    shapClinics_train = torch.from_numpy(feature.values).long().cpu()#.requires_grad_()
    ...
    train_dl = DataLoader(train_dataset, args.batch_size, shuffle=False, collate_fn=partial(FTT, cat_ind=[args.cat_ind], cont_ind=[args.cont_ind]), 
                          pin_memory=True, num_workers=args.num_workers, drop_last=True, sampler=sampler)
    val_dl = DataLoader(valid_dataset, args.batch_size, shuffle=False, collate_fn=partial(FTT, cat_ind=[args.cat_ind], cont_ind=[args.cont_ind]), 
                        pin_memory=True, num_workers=args.num_workers, drop_last=False)
    
    model = FTT_trainer(args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset) # label_names=['HematomaExpansion', 'nonHE']
    ...
    shapClinics = torch.from_numpy(feature.values).long().cpu()#.requires_grad_()
    ...
    test_dl_paired = DataLoader(dataset, batch_size=5, shuffle=False, num_workers=12, 
                                collate_fn=partial(FTT, cat_ind=[args.cat_ind], cont_ind=[args.cont_ind])
                                )
    ...
    model = FTT_trainer.load_from_checkpoint(checkpoint_callback.best_model_path, args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset)
    ...
    save_auc_plot(model.test_results["y_gt"][:, 0], model.test_results["preds"], save_path)
    save_calibration_plot(model.test_results["y_gt"][:, 0], model.test_results["preds"], save_path)
    model.model.eval()
    # 예측 함수 정의
    def predict_proba(X):
        X = torch.tensor(X, dtype=torch.float32)
        with torch.no_grad():
            outputs = model.model(X[:, args.cat_ind].long(), X[:, args.cont_ind])
            outputs = F.sigmoid(outputs.squeeze(-1)).numpy()
        return np.column_stack([1 - outputs, outputs])
    explainer = lime_tabular.LimeTabularExplainer(shapClinics_train, feature_names=feature.columns, class_names=['HematomaExpansion', 'nonHE'], categorical_features=args.cat_ind, discretize_continuous=False)
    i = 0  # 첫 번째 샘플 선택 # 추후, 모든 test sample에 대해 wandb에 logging하거나, Hematoma expansion==1 인 애들만 한 plot에 모아보기
    exp = explainer.explain_instance(shapClinics[i].numpy(), predict_proba, num_features=len(shapClinics[i]))
    fig = exp.as_pyplot_figure()
    plt.savefig(os.path.join(save_path, 'lime_plot.png'))
    ...
    #         self.model = model
    #     def forward(self, x):
    #         # Assuming x is a tuple (train_cat, train_cont)
    #         cat, cont = x[:, args.cat_ind], x[:, args.cont_ind]
    #         return self.model(cat, cont)  # Adjust based on model's expectations
    # explainer = shap.DeepExplainer(SHAPModelWrapper(model.model), shapClinics_train) # (shapModelWrapper(self.model.ehr_model.cpu()), x)
