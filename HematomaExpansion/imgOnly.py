...
from models import imgOnlyTrainer
...
from utils import (
    ...
    create_transforms, maxSeg_repeat3, maxSeg_around3, maxSeg_around3_sample,# maxSeg_around3_test, collate_like_drfuse,
...
if __name__ == '__main__':
    ...
    args.exp_name = input("Specify the note for this run: ")
    ...
    train_dataset.start()
    valid_dataset.start()
    train_dl = DataLoader(train_dataset, args.batch_size, shuffle=False, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size, test=True),#partial(maxSeg_around3, img_size=args.img_size),
                          pin_memory=True, num_workers=args.num_workers, drop_last=True, sampler=sampler)                              # test=True 인 이유는 image sampling이랑 augmentation잘 됐나 pid 별로 보려고
    val_dl = DataLoader(valid_dataset, args.batch_size, shuffle=False, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size, test=True),#partial(maxSeg_around3, img_size=args.img_size),
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=save_path,
        filename='{epoch}-{val_PRAUC_avg_over_dxs/final:.2f}-{val_loss/total:.2f}',
    ...
    test_dl_paired = DataLoader(dataset, batch_size=5, shuffle=False, num_workers=12, collate_fn=partial(maxSeg_around3_sample, img_size=args.img_size, test=True))#partial(maxSeg_around3, img_size=args.img_size, test=True))
    model = imgOnlyTrainer.load_from_checkpoint(checkpoint_callback.best_model_path, args=args, label_names=['HematomaExpansion', 'nonHE'], train_dataset=train_dataset, valid_dataset=valid_dataset)
